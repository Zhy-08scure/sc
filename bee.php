...
const
scr = "beefct", host = "https://beefaucet.org/",
ref = "/?r=whyredxcb@gmail.com",
reg = "https://cutt.ly/keXxJm0Z", rc = ' [Xevil]', hp = '     ',
...
function status() {
   ...
   $status = explode('#', explode('#beefct:', $r)[1])[0];
   $link = $res[1][4];
   $a = 100;
   for ($i = 0; $i <= $a; $i++) {
...
db:
$r = run("https://beefaucet.org/btc-faucet/?r=whyredxcb@gmail.com", h());
$cfw = cfw($r);
if($cfw['cf']){
   cok(1); goto ck;
}
...
$r = run(host, h());
$url = explode("window.location = 'https://beefaucet.org/", $r);
...
$coin = explode("/", explode("window.location = 'https://beefaucet.org/", $r)[$pilih])[0];

gas:
while (1) {
   $r = run(host.$coin.ref, h());
   if (preg_match('/Just a moment.../', $r)) {
         cok(1);wc();system("termux-open-url ".reg);
         save('Cookie');system('clear');continue;
      }
